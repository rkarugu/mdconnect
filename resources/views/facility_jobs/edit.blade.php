...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6">
        <a href="{{ route('facility_jobs.show', $job) }}" class="text-blue-600 hover:text-blue-800">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <form action="{{ route('facility_jobs.update', $job) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="specialty_id" id="specialty_id" 
                            ...
                            @foreach($specialties as $specialty)
                                <option value="{{ $specialty->id }}" {{ old('specialty_id', $job->specialty_id) == $specialty->id ? 'selected' : '' }}>
                                    {{ $specialty->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" name="required_experience_years" id="required_experience_years" 
                        value="{{ old('required_experience_years', $job->required_experience_years) }}" min="0" step="1" 
                ...
                <div>
                    ...
                    <input type="number" name="hourly_rate" id="hourly_rate" 
                        value="{{ old('hourly_rate', $job->hourly_rate) }}" min="0" step="0.01" 
                ...
                <div class="mb-4">
                    <div class="flex items-center">
                        <input type="checkbox" name="is_recurring" id="is_recurring" value="1" 
                            {{ old('is_recurring', $job->is_recurring) ? 'checked' : '' }} class="mr-2" onchange="toggleShiftType()">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="shift_type" id="shift_type" 
                            ...
                            required>
                            <option value="day" {{ old('shift_type', $job->shift_type) == 'day' ? 'selected' : '' }}>Day Shift</option>
                            <option value="night" {{ old('shift_type', $job->shift_type) == 'night' ? 'selected' : '' }}>Night Shift</option>
                            <option value="evening" {{ old('shift_type', $job->shift_type) == 'evening' ? 'selected' : '' }}>Evening Shift</option>
                            <option value="custom" {{ old('shift_type', $job->shift_type) == 'custom' ? 'selected' : '' }}>Custom Shift</option>
                        </select>
                        @error('shift_type')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="slots_available" id="slots_available" 
                            value="{{ old('slots_available', $job->slots_available) }}" min="1" step="1" 
